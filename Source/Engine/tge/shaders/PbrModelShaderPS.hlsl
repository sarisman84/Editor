...
PixelOutput main(ModelVertexToPixel input)
{
    ...
    float3 lightColor = 0; // <- The sum of all point lights.
    for (unsigned int p = 0; p < NumberOfLights; p++)
    {
        
        if (myLights[p].InnerAngle == 0 || myLights[p].OuterAngle == 0)
        {
            lightColor += EvaluatePointLight(
			       diffuseColor,
                   specularColor,
                   pixelNormal,
                   roughness,
			       myLights[p].Color.rgb,
                   myLights[p].Color.w,
                   myLights[p].Range,
                   myLights[p].Position.xyz,
			       toEye.xyz,
                   input.worldPosition.xyz);
        }
        else
        {
            lightColor += EvaluateSpotLight(
			        diffuseColor,
                    specularColor,
                    pixelNormal,
                    roughness,
                    ...
                    myLights[p].Range,
                    myLights[p].Position.xyz,
                    myLights[p].FacingDirection.xyz,
                    myLights[p].OuterAngle,
                    myLights[p].InnerAngle,
			        toEye.xyz,
                    input.worldPosition.xyz);
        }
    ...
    float3 radiance = ambiance + directionalLight + lightColor + emissiveAlbedo;
