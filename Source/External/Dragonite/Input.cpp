...
Tga::Vector2<float>& CommonUtilities::GClient::GetLeftStick() noexcept
{
	return myLeftStick;
}

Tga::Vector2<float>& CommonUtilities::GClient::GetRightStick() noexcept
{
	return myRightStick;
}
...
Tga::Vector2<float>& CommonUtilities::GClient::GetDeadzone() noexcept
{
	return myDeadzone;
}
...
{
	myCurrentGamepads.push_back(GClient(static_cast<unsigned int>(myCurrentGamepads.size()) + 1));
...
Tga::Vector2<int> CommonUtilities::Mouse::ourPastMousePosition;


Tga::Vector2<float> CommonUtilities::Mouse::ourMouseDelta;
Tga::Vector2<int> CommonUtilities::Mouse::ourMousePosition;
HCURSOR CommonUtilities::Mouse::ourInstance;

CommonUtilities::Mouse::CaptureState CommonUtilities::Mouse::ourCaptureState;
void CommonUtilities::Mouse::Init()
{
	ourInstance = LoadCursor(NULL, IDC_ICON);
}
...
void CommonUtilities::Mouse::UpdateEvents(HWND aWindowsIns, UINT aMessage, WPARAM /*anWParam*/, LPARAM /*anLParam*/)
{
	...
	if (ourCaptureState != CaptureState::None)
	{
		RECT clipRect;

		GetClientRect(aWindowsIns, &clipRect);
		...
		MapWindowPoints(aWindowsIns, nullptr, &upperLeft, 1);
		MapWindowPoints(aWindowsIns, nullptr, &lowerRight, 1);
...
void CommonUtilities::Mouse::EndFrame()
{
	...
	ourMouseDelta = Tga::Vector2f(diff);
...
const Tga::Vector2< int> CommonUtilities::Mouse::GetMousePosition()
{
	return ourMousePosition;
}

const Tga::Vector2<float> CommonUtilities::Mouse::GetMouseDelta()
{
	return ourMouseDelta;
}


void CommonUtilities::Mouse::SetVisibility(const bool& aState)
{
	int d = ShowCursor(aState);
	
	while (aState ? d <= 1 : d >= 0)
	{
		d = ShowCursor(aState);
	}
...
void CommonUtilities::Mouse::SetCaptureState(const CaptureState aState)
{
	ourCaptureState = aState;
}
