...
namespace Tga
{
	...
	public:
		...
		ImGuiInterface& GetMyIMGUIInterface() { return *myImguiInterFace; }
		...
		FORCEINLINE const Color& GetClearColor() const { return myClearColor; }
		...
		inline void AddWinProcCallback(callback_function_wndProc aCallback) {
			myCustomWinProcCallback = aCallback;
		}


		void OnCustomWinProcCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
			if (myCustomWinProcCallback)
				myCustomWinProcCallback(hWnd, message, wParam, lParam);
		}
	...
	private:
		...
		//CUSTOM
		callback_function_wndProc myCustomWinProcCallback;
	};
}
